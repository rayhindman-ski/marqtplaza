...
function currentUserId(req: Parameters<typeof getAuth>[0], res: {
...
router.get("/saved-events", async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.status(500).json({ error: "Saved events are temporarily unavailable." });
  }
});
...
router.post("/saved-events/sync", async (req, res): Promise<void> => {
  ...
  try {
    await db.transaction(async (tx) => {
      if (body.removeEventIds.length > 0) {
        ...
        for (const eventId of body.removeEventIds) {
          await tx.insert(savedEventTombstonesTable)
            .values({ userId, eventId })
            .onConflictDoUpdate({
              target: [savedEventTombstonesTable.userId, savedEventTombstonesTable.eventId],
      ...
      if (body.removeAlertFingerprints.length > 0) {
        await tx.delete(savedEventAlertsTable).where(and(
          eq(savedEventAlertsTable.userId, userId),
          inArray(savedEventAlertsTable.fingerprint, body.removeAlertFingerprints),
        ));
      }

      const migrationIds = body.migrationEvents.map((event) => event.eventId);
      const tombstonedMigrationIds = migrationIds.length > 0
        ? new Set((await tx.select({ eventId: savedEventTombstonesTable.eventId })
          .from(savedEventTombstonesTable)
          .where(and(
            eq(savedEventTombstonesTable.userId, userId),
            inArray(savedEventTombstonesTable.eventId, migrationIds),
          ))).map((row) => row.eventId))
        : new Set<string>();
      ...
      const eventsToUpsert = [
        ...body.migrationEvents.filter((event) => !tombstonedMigrationIds.has(event.eventId)),
        ...explicitEvents,
      ].filter((event) => !body.removeEventIds.includes(event.eventId));

      if (explicitEvents.length > 0) {
        await tx.delete(savedEventTombstonesTable).where(and(
          eq(savedEventTombstonesTable.userId, userId),
          inArray(savedEventTombstonesTable.eventId, explicitEvents.map((event) => event.eventId)),
        ));
      }
      for (const event of eventsToUpsert) {
        await tx.insert(savedEventSnapshotsTable)
          .values({ userId, eventId: event.eventId, snapshot: event.snapshot })
          .onConflictDoUpdate({
            target: [savedEventSnapshotsTable.userId, savedEventSnapshotsTable.eventId],
            set: { snapshot: event.snapshot, updatedAt: new Date() },
          });
      }

      const alertEventIds = [...new Set(body.alerts.map((item) => item.eventId))];
      const savedAlertEventIds = alertEventIds.length > 0
        ? new Set((await tx.select({ eventId: savedEventSnapshotsTable.eventId })
          .from(savedEventSnapshotsTable)
          .where(and(
            eq(savedEventSnapshotsTable.userId, userId),
            inArray(savedEventSnapshotsTable.eventId, alertEventIds),
          ))).map((row) => row.eventId))
        : new Set<string>();
      for (const item of body.alerts.filter((alert) => savedAlertEventIds.has(alert.eventId))) {
        await tx.insert(savedEventAlertsTable)
          .values({
            userId,
          ...
          .onConflictDoUpdate({
            target: [savedEventAlertsTable.userId, savedEventAlertsTable.fingerprint],
            set: { eventId: item.eventId, alert: item.alert, updatedAt: new Date() },
          });
      }
    });
    ...
    res.status(500).json({ error: "Saved events could not be synchronized." });
  }
});
