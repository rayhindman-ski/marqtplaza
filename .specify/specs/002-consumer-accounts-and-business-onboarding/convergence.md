...
6. The real Clerk development-session pass for the authenticated
   ...
   exercised.~~ Resolved 2026-09-15: real sign-up with e-mail-code
   verification, and the expired / already-used sign-in link and consumed
   verification step, are recorded in the US2 row. Still owed before the
   first production flag: real SMTP delivery to a third-party inbox.
...
| US2 preferences | Implemented 2026-09-14: `PATCH /account/preferences` (expectedRevision, controlled-ID validation → 400 `VALIDATION_FAILED` with `not_in_controlled_list`, 409 `VERSION_CONFLICT` with `expectedVersion`, omitted fields unchanged / empty arrays clear, locale on `app_users`), `POST /account/onboarding/complete` (idempotent, skip creates no preference row), `GET`/`POST /account/consents` (append-only ledger, purposes `marketing_updates` and `research_contact`, client must echo the current notice version `draft-2026-09`, `support`/`system` sources rejected from clients, writes require a verified identity). Web: `/account/voorkeuren` (checkbox groups from `/account/options`, save/skip/cancel, draft kept in session storage across refresh, conflict keeps draft and reloads revision, error summary receives focus, NL/EN switch keeps the draft), `AccountPage` with preference summary, per-purpose consent controls, explicit account / research registration / saved data / consent scopes, `returnPath.ts` allowlist (`terug`), Clerk sign-in/sign-up redirects derive from the sanitised return path, `OnboardingPage` marker mirrors server state and points to the separate preference step. Account creation still writes no registration, subscription, or consent. | `pnpm --filter @workspace/api-server run test:account-foundation` → 24 pass, 0 fail (incl. six concurrent revision-0 writers → one 200, five 409; consent `current` derived from the newest row beyond a 200-entry history page). `tsx --test src/lib/returnPath.test.ts` → 5 pass. `playwright test` → 20 pass (8 new in `e2e/account-preferences.spec.ts`: anonymous discovery prompt-free, signed-out redirect with safe `terug`, save + language switch + refresh + resume, skip with external URL rejected, stale revision conflict, failed save keeps input and focuses the error, account page scopes and consent recording, disabled gate state; discovery and saved-events regressions unchanged). `pnpm run typecheck` clean. Live dev: `/api/readiness` → accounts true, unauthenticated `PATCH /api/account/preferences` → 401 `AUTH_REQUIRED`. Live Clerk development-session smoke on 2026-09-14 used a disposable, backend-created and email-verified test identity against the proxied development URL (no production data): sign-in loaded `/account`; `/account/voorkeuren?terug=/deals` loaded real `/api/account/options` and `/api/account/me`, saved two controlled neighbourhoods, refreshed with both selections intact, `/account` displayed the saved names, sign-out returned to discovery, and the signed-out preferences route redirected to `/sign-in?terug=...`. Observed API responses were 200 for options/me/consents, 200 for `PATCH /api/account/preferences`, and 200 for `POST /api/account/onboarding/complete`; no browser errors occurred. The real sign-up attempt on 2026-09-14 loaded the Clerk widget but headless Chromium did not complete its bot-protection challenge, no submit request or verification message was observed in the disposable inbox within 120 seconds, and the expired verification-link case was not executed. **Real sign-up completed 2026-09-15** with the opt-in spec `e2e/clerk-live-signup.spec.ts` (`CLERK_LIVE_SIGNUP=1 CLERK_LIVE_BASE_URL=https://<workspace dev domain>`, system Chromium via `PLAYWRIGHT_CHROMIUM_EXECUTABLE`): environment = Clerk development instance `enjoyed-buffalo-2094.clerk.accounts.dev` (`pk_test`, Turnstile `smart` widget, `captcha_enabled: true`), bot protection satisfied with a short-lived Backend API testing token (`POST /v1/testing_tokens`, appended as `__clerk_testing_token` to every Frontend API request — the `@clerk/testing` mechanism) while the Turnstile script was blocked so clerk-js submitted with `captcha_error` instead of waiting on the interactive checkbox; identity = a Clerk test identity `buurtplaza.e2e.<timestamp>+clerk_test@example.com` (Clerk's controlled development inbox: no message leaves Clerk, the e-mail-code verification accepts `424242`). Observed: `POST /v1/client/sign_ups` → `missing_requirements` with `email_address` unverified → `prepare_verification` (`email_code`) → app step `/sign-up/verify-email-address` → `attempt_verification` → `complete` + session touch → landed on `/account/voorkeuren?terug=%2Factiviteiten%2Fden-haag%2Fzeeheldenkwartier%3Fbron%3De2e` with the preference form (save button) rendered and `GET /api/account/me` → 200 `capabilities.isVerified: true`, `onboardingCompleted: false`. Clerk Backend API lists the created users with `email_addresses[].verification.status: verified` (`strategy: email_code`); three such test users exist from the runs (`bp.…`, `buurtplaza.e2e.…`). **Defect found and fixed by this run:** Clerk's internal step navigation (`/sign-up/verify-email-address`) used a bare path through `routerPush`, so `terug` was dropped before the redirect target was computed and the first live run landed on `/account/voorkeuren` without it; `carryReturnPath` in `returnPath.ts` now re-attaches the sanitised `terug` to `/sign-in*` and `/sign-up*` navigations (unit-tested, `pnpm --filter @workspace/buurtgids run test` → 7 unit + 44 e2e pass, live spec skipped without the flag). Not covered: real SMTP delivery to a third-party inbox (test identities never send mail). **Expired and consumed credentials verified 2026-09-15** with the opt-in spec `e2e/clerk-verification-recovery.spec.ts` (`CLERK_LIVE_RECOVERY=1 CLERK_LIVE_BASE_URL=https://<workspace dev domain>`, same development instance, database = the shared development database inspected before and after because the run only reads through the running dev server; screenshots in `test-results/clerk-recovery/`). Scope note: this instance verifies e-mail with a one-time code only (`verifications: ["email_code"]`), so no e-mail *verification link* exists to expire; the genuinely Clerk-issued links are sign-in tickets, and the verification step itself is `/sign-up/verify-email-address`. Three cases were opened against the dev app: (1) a genuinely expired sign-in link (Backend API sign-in token, `expires_in_seconds: 1`, opened 3 s later on `/sign-in?__clerk_ticket=…&terug=/deals`) → Frontend API `POST /v1/client/sign_ins` 400 `ticket_expired_code`; visible Clerk state: sign-in card with the message “This link has expired. Please start again or request a new link.” plus the e-mail/password form, Google button and “Sign up” link as recovery; URL stayed on `/sign-in`, `Clerk.session` null. (2) An already-consumed sign-in link: first use in a fresh context signed in a test identity and landed on `/deals` (session recorded, then signed out); a second fresh context opening the same link → 400 `sign_in_token_already_used_code`, card message “This sign in token has already been used. Each token can only be used once.” with the same recovery form, no session. (3) The e-mail verification step: a wrong code first showed “Incorrect code” with the “Didn't receive a code? Resend (30)” action and the Continue button (recoverable); the test-identity code then completed the sign-up (`/account/voorkeuren?terug=/deals`), the browser signed out, and the consumed `/sign-up/verify-email-address?terug=%2Fdeals` URL was reopened. **Defect found and fixed by this run:** Clerk renders an empty card for that stale step when no sign-up is in progress (reproduced on Clerk's own Account Portal `…accounts.dev/sign-up/verify-email-address`, blank after 12 s), which was a dead end with no recovery action. `useStaleSignUpStepRecovery` in `App.tsx` now sends a signed-out visitor of `/sign-up/verify-*` with no sign-up attempt (`clerk.client.signUp.id` absent) back to `/sign-up`, keeping `terug`; after the fix the replay landed on `/sign-up?terug=%2Fdeals` with the “Create your account” form, no code input, no session. Local side effects: `app_users`, `consumer_preferences`, `account_consent_events` and `account_requests` counts were 0/0/0/0 before the runs; afterwards only `app_users` rows for the three test identities whose sign-up *completed genuinely* existed (created by `GET /api/account/me` on the preference step), none for the expired/consumed visits or for the ticket sign-in that landed on `/deals`; those three rows were deleted again (baseline restored to 0). Clerk side: three additional `buurtplaza.recovery.<timestamp>+clerk_test@example.com` test users exist. Regression: `pnpm --filter @workspace/buurtgids run test` → 7 unit + 44 e2e pass, 4 skipped (both live specs skipped without their flags); `pnpm run typecheck` clean after regenerating the shared API client. **Clerk cards localized 2026-09-15:** the recovery run above showed Clerk's cards in English only while the app was Dutch. `ClerkProvider` now receives a `localization` that follows the stored app language (`clerkLocalizationFor` in `src/lib/clerkLocalization.ts`: `nlNL` from `@clerk/localizations` plus Dutch overrides for the error codes that package leaves untranslated — `ticket_expired_code`, `ticket_invalid_code`, `form_code_incorrect`, and the untyped `sign_in_token_already_used_code`; `enUS` otherwise). Language changes reach the provider through `persistLanguage`/`useStoredLanguage`, so every page's toggle now uses one helper. Re-run with `CLERK_LIVE_LANGUAGE=nl` (new spec option; it seeds the stored language before load and asserts Dutch text): expired link → “Deze link is verlopen. Begin opnieuw of vraag een nieuwe link aan.”, consumed link → “Deze inloglink is al gebruikt. Elke link werkt maar één keer. …”, wrong code → “De ingevoerde code is onjuist.” with “Niet ontvangen? Opnieuw verzenden”, all with the Dutch sign-in/sign-up form as recovery; the same four live tests also pass with `CLERK_LIVE_LANGUAGE=en`. The seven `app_users` rows created by these runs were deleted again (baseline 0/0/0/0). Regression: 7 unit + 47 e2e pass, 4 skipped; workspace typecheck clean. Gates: Q1 taxonomy still `provisional-2026-09`; Q6 consent notice is a draft version; Q7 unresolved — the research registration remains a separate, voluntary step and sign-up lands on the optional preference step only while the flag is on. |
